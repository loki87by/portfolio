...
function Popup(props) {
  ...
  // **DOM
  return (
    <section className={`Popup ${props.isOpen && "Popup_opened"}`} id={props.popupType}>
      <div className="Popup__container" style={{ maxHeight: `${props.isTimedPopup && "50vw" }`}} name={props.popupType}>
        <h1 className="Popup__text">{title}</h1>
        {props.isTimedPopup ? '' :
          <figure className="Popup__smile">
            <div className="Popup__smile-bigcircle" style={{ borderColor: picColor }}></div>
            <div className="Popup__smile-smallcircle Popup__smile-smallcircle_left" style={{ borderColor: picColor, backgroundColor: picColor }}></div>
            <div className="Popup__smile-smallcircle Popup__smile-smallcircle_right" style={{ borderColor: picColor, backgroundColor: picColor }}></div>
            <div style={{ borderTopColor: picColor }} className={`${props.popupType === 'win' && "Popup__smile-mimic_type_win"} ${props.popupType === 'loose' && "Popup__smile-mimic_type_loose"} Popup__smile-mimic`}></div>
          </figure>}
        <div className="Popup__buttons" style={{ flexDirection: `${props.isTimedPopup && "column" }`}}>
          {props.popupType === 'win' ?
            <button type="button" className="Popup__button" onClick={props.onClose}>{next}</button>
            : props.isTimedPopup ?
              <>
                <button type="button"
                disabled={props.isTimerEdited ? null : 'disabled'}
                style={{ backgroundColor: `${props.isTimerEdited ? "goldenrod" : ''}` }}
                className={`Popup__button Popup__button_vertical ${!props.isTimerEdited && "Popup__button_disabled"}`}
                onClick={props.popupType === 'oclock' ? props.newTimer : props.newTimeInterval}>
                {props.isTimerEdited ?
                  props.popupType === 'oclock' ?
                    translation.remindAt : next
                  : next}</button>
                {props.popupType === 'oclock' ?
                  <input className="Popup__time-input" onChange={e => props.handleEditTimer(e)} type="time" required /> :
                  <select className="Popup__time-input" defaultValue={translation.mins} id="timerChanger" onChange={props.getNewTimerValue} required>
