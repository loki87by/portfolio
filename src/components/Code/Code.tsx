...
function Code(props: CodeProps): ReactElement {
  ...
  let result = false;
  let end = " ";
  let lastAfterPunkt: string | undefined = undefined;
  ...
  useEffect(() => {
    ...
    Array.from(lines).forEach((item) => {
      const max = (item as HTMLElement).offsetWidth;
      ...
      if (max < props.width * 0.86 - props.scrollbarWidth) {
        ...
        children.forEach((el) => {
          ...
          if (!maxWidth) {
            if (current + (el as HTMLElement).offsetWidth < max) {
              current += (el as HTMLElement).offsetWidth;
            } else {
  ...
  function pointsChecker(keyword: string) {
    if (keyword === ".") {
  ...
  function importChecker(keyword: string) {
    if (keyword === "{") {
      multiplyImport = true;
  ...
  function openBracketChecker(keyword: string) {
    if (keyword === "(" || keyword === "((") {
      openBracket = true;
  ...
  function closeBracketChecker(keyword: string): ReactElement {
    if (keyword === ")") {
      openBracket = false;
      end = " ";
      return <span>)</span>;
  ...
  function openSquareBracketChecker(keyword: string): ReactElement {
    if (keyword === "[") {
      return <span>[</span>;
    }
    const arr = keyword.split("[");
  ...
  function closeSquareBracketChecker(keyword: string): ReactElement {
    if (keyword === "]") {
      return <span>]</span>;
    }
    const arr = keyword.split("]");
  ...
  function semicolonChecker(keyword: string): ReactElement {
    end = "";
  ...
  function commaChecker(keyword: string): ReactElement {
    return (
      <>
        {wordChecker(keyword.replace(",", ""))}
        <span>, </span>
  ...
  function greenWordsChecker(keyword: string) {
    isGreen = false;
  ...
  function wordChecker(word: string) {
    if (result) {
      result = false;
  ...
  function lineChecker(line: string) {
  ...
  const html = <div className="Code">{splitCode(CODE)}</div>;
